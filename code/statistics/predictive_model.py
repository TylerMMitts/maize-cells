...
for cell_file in cell_files:
    ...
    image_name = root_folder  # This should match the image_name in master_summary
    ...
    try:
        ...
        # This is the innermost cortical file adjacent to the stele
        ...
        if len(first_file_cells) > 0:
            ...
            first_file_data[image_name] = {
                ...
                # Also capture the total cells in this file for reference
            ...
            # Check if there are any cells at all
            if len(df_cells) > 0:
...
# We need to average across quadrants for each root
...
df_clean = df_agg.dropna(subset=['first_file_avg_area_um2', 'root_radius_um', 'file_count'])

# Remove any rows with unrealistic values (zero or negative)
df_clean = df_clean[df_clean['first_file_avg_area_um2'] > 0]
df_clean = df_clean[df_clean['root_radius_um'] > 0]
...
print(f"  Root radius (µm): {df_clean['root_radius_um'].min():.1f} - {df_clean['root_radius_um'].max():.1f}")
...
# Response variables
y_train_radius = df_clean['root_radius_um'].values[train_idx]
y_test_radius = df_clean['root_radius_um'].values[test_idx]
...
# 6a. Model for Root Radius (Linear)

model_radius = LinearRegression()
model_radius.fit(X_train, y_train_radius)

# Predictions
y_train_pred_radius = model_radius.predict(X_train)
y_test_pred_radius = model_radius.predict(X_test)

# Performance metrics
train_r2_radius = r2_score(y_train_radius, y_train_pred_radius)
test_r2_radius = r2_score(y_test_radius, y_test_pred_radius)
train_rmse_radius = np.sqrt(mean_squared_error(y_train_radius, y_train_pred_radius))
test_rmse_radius = np.sqrt(mean_squared_error(y_test_radius, y_test_pred_radius))

print(f"Training R²: {train_r2_radius:.4f}")
print(f"Testing R²:  {test_r2_radius:.4f}")
print(f"Training RMSE: {train_rmse_radius:.1f} µm")
print(f"Testing RMSE:  {test_rmse_radius:.1f} µm")
print(f"Coefficient: {model_radius.coef_[0]:.4f}")
print(f"Intercept: {model_radius.intercept_:.1f}")
...
# Root radius model
cv_scores_radius = cross_val_score(model_radius, X_train, y_train_radius, cv=kf, scoring='r2')
print(f"\nRoot Radius Model:")
print(f"  Cross-validation R²: {cv_scores_radius.mean():.4f} ± {cv_scores_radius.std():.4f}")
...
fig.suptitle('Multivariate Predictive Model: Cortical Cell Anatomy\n(Predictor: First File Cell Area via cell_file_derivative == 0)', 
             fontsize=16, fontweight='bold')

# 8a. Root Radius: Observed vs Predicted (Testing)
ax1 = axes[0, 0]
ax1.scatter(y_test_radius, y_test_pred_radius, alpha=0.5, s=20, label='Testing', color='green')
ax1.plot([y_test_radius.min(), y_test_radius.max()], 
         [y_test_radius.min(), y_test_radius.max()], 
         'r--', linewidth=2, label='Perfect Prediction')
ax1.set_xlabel('Observed Root Radius (µm)', fontsize=11)
ax1.set_ylabel('Predicted Root Radius (µm)', fontsize=11)
ax1.set_title(f'Root Radius: Testing (R² = {test_r2_radius:.3f})', fontsize=12)
ax1.legend()
...
# 8f. Scatter plot of first file area vs root radius
ax6 = axes[1, 2]
ax6.scatter(X_train, y_train_radius, alpha=0.4, s=15, label='Training', color='blue')
ax6.scatter(X_test, y_test_radius, alpha=0.4, s=15, label='Testing', color='green')
# Add regression line
x_range = np.linspace(X.min(), X.max(), 100)
y_range = model_radius.predict(x_range.reshape(-1, 1))
ax6.plot(x_range, y_range, 'r-', linewidth=2, label='Regression')
ax6.set_xlabel('First File Cell Area (µm²)', fontsize=11)
ax6.set_ylabel('Root Radius (µm)', fontsize=11)
ax6.set_title('First File Area vs Root Radius', fontsize=12)
ax6.legend()
ax6.grid(True, alpha=0.3)

plt.tight_layout()

fig_path = output_dir / 'predictive_model_results.png'
plt.savefig(fig_path, dpi=300, bbox_inches='tight')
print(f"Figure saved to: {fig_path}")

# Create summary dataframe
results_summary = pd.DataFrame({
    'Model': ['Root Radius', 'File Count', 'Cell Area'],
    'Relationship': ['Linear', 'Linear', f'Degree {best_degree}'],
    'Train_R2': [train_r2_radius, train_r2_files, results_area_best['train_r2']],
    'Test_R2': [test_r2_radius, test_r2_files, results_area_best['test_r2']],
    'Train_RMSE': [train_rmse_radius, train_rmse_files, results_area_best['train_rmse']],
    'Test_RMSE': [test_rmse_radius, test_rmse_files, results_area_best['test_rmse']],
    'CV_R2_mean': [cv_scores_radius.mean(), cv_scores_files.mean(), cv_scores_area.mean()],
    'CV_R2_std': [cv_scores_radius.std(), cv_scores_files.std(), cv_scores_area.std()]
})
...
coefs_df = pd.DataFrame({
    'Model': ['Root Radius', 'File Count'],
    'Coefficient': [model_radius.coef_[0], model_files.coef_[0]],
    'Intercept': [model_radius.intercept_, model_files.intercept_]
})
...
Model Performance:
-----------------
1. Root Radius (Linear):
   • Testing R²: {test_r2_radius:.4f}
   • Testing RMSE: {test_rmse_radius:.1f} µm
   • CV R²: {cv_scores_radius.mean():.4f} ± {cv_scores_radius.std():.4f}
