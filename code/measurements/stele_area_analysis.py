...
OUTPUT_FOLDER = str(PROJECT_ROOT / "results" / "stele_area_analysis")
N_BINS = 5 
MIN_CELLS_PER_IMAGE = 10 
N_PROFILE_BINS = 20 

def find_all_measurement_files_with_stele(measurements_folder):
    ...
    for file_info in all_files:
        ...
        if summary is not None and summary.get('stele_area_um2', 0) > 0:
            ...
            if df is not None and len(df) >= MIN_CELLS_PER_IMAGE:
                image_data.append({
                    ...
                    'summary': summary,
                    'df': df,
                    'stele_area_um2': summary['stele_area_um2'],
                    'root_radius_um': summary.get('root_radius_um', 0),
                    'quadrant': summary.get('quadrant', 'unknown')
                })
    
    return image_data

def bin_images_by_stele_area(image_data, n_bins=N_BINS):
    stele_areas = [img['stele_area_um2'] for img in image_data if img['stele_area_um2'] > 0]
    
    if len(stele_areas) < n_bins:
        print(f"Only {len(stele_areas)} images with stele area, cannot create {n_bins} bins")
        return {}, []
    
    # Create bins based on percentiles (evenly distributed)
    percentiles = np.linspace(0, 100, n_bins + 1)
    bin_edges = np.percentile(stele_areas, percentiles)
    bin_edges[0] = -np.inf
    bin_edges[-1] = np.inf
    ...
    bin_names = [f'Stele {i+1}' for i in range(n_bins)]
    ...
    for i in range(n_bins):
        ...
        for img in image_data:
            if img['stele_area_um2'] > 0 and low <= img['stele_area_um2'] < high:
...
def plot_continuous_color_profiles(binned_images, bin_ranges, output_folder):
    ...
    ax.set_title('Cell Size vs Normalized Distance from Center\n'
                'Colored by Stele Area (5 equal bins)', 
...
def perform_regression_analysis(image_data, output_folder):
    ...
    for img in image_data:
        df = img['df'].copy()
        df['stele_area'] = img['stele_area_um2']
    ...
    mean_stele = combined_df['stele_area'].mean()
    
    # Step 2: Create centered variables
    combined_df['radius_centered'] = combined_df['radius_um'] - mean_radius
    combined_df['stele_centered'] = combined_df['stele_area'] - mean_stele
    
    # Step 3: Create interaction term from centered variables
    combined_df['radius_stele_centered'] = combined_df['radius_centered'] * combined_df['stele_centered']
    
    # Step 4: Build the X matrix with centered variables
    X_centered = combined_df[['radius_centered', 'stele_centered', 'radius_stele_centered']].copy()
    ...
    with open(os.path.join(output_folder, '02_regression_results_centered.txt'), 'w', encoding='utf-8') as f:
        f.write("REGRESSION ANALYSIS: Cell Area vs Radius and Stele Area\n")
        f.write("MODEL: area = b0 + b1*(radius - mean_radius) + b2*(stele - mean_stele) + b3*(radius - mean_radius)*(stele - mean_stele)\n")
        
        f.write(f"CENTERING VALUES:\n")
        f.write(f"  Mean radius: {mean_radius:.4f} µm\n")
        f.write(f"  Mean stele area: {mean_stele:.4f} µm²\n\n")
        ...
        f.write(f"  -> Predicted cell area when radius and stele area are at their AVERAGE values: {coeffs['const']:.2f} µm²\n")
        
        # Radius effect
        if 'radius_centered' in coeffs.index:
            f.write(f"\nb1 (Radius effect, at average stele area): {coeffs['radius_centered']:.4f} (p={pvals['radius_centered']:.4e})\n")
            f.write(f"  -> At the average stele area, a 1 µm increase in radius is associated with a {coeffs['radius_centered']:.4f} µm² increase in cell area\n")
        
        # Stele area effect
        if 'stele_centered' in coeffs.index:
            f.write(f"\nb2 (Stele area effect, at average radius): {coeffs['stele_centered']:.4e} (p={pvals['stele_centered']:.4e})\n")
            f.write(f"  -> At the average radius, a 100,000 µm² increase in stele area is associated with a {coeffs['stele_centered']*100000:.2f} µm² increase in cell area\n")
        
        # Interaction effect
        if 'radius_stele_centered' in coeffs.index:
            f.write(f"\nb3 (Interaction effect): {coeffs['radius_stele_centered']:.4e} (p={pvals['radius_stele_centered']:.4e})\n")
            if pvals['radius_stele_centered'] < 0.05:
                f.write(f"  -> Significant interaction: The effect of radius on cell area changes as stele area changes\n")
                f.write(f"  -> For every 1 µm² increase in stele area (above average), the effect of radius changes by {coeffs['radius_stele_centered']:.4e} µm²\n")
            else:
                f.write(f"  -> No significant interaction: The effect of radius does not depend on stele area\n")
...
def main():
    ...
    image_data = find_all_measurement_files_with_stele(MEASUREMENTS_FOLDER)
    
    if not image_data:
        print("No image data found with stele area measurements!")
        print("Please check that measurement files and summaries exist.")
        return

    binned_images, bin_ranges = bin_images_by_stele_area(image_data, N_BINS)
